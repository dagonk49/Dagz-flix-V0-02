...
async function handleAuthLogin(req) {
  ...
  if (!username && username !== '') return jsonResponse({ success: false, error: 'Identifiants requis' }, 400);
...
async function handleDiscover(req) {
  ...
  const res = await fetch(`${config.jellyseerrUrl}/api/v1/discover/${endpoint}?page=${page}&language=fr&sortBy=popularity.desc`, {
    headers: { 'X-Api-Key': config.jellyseerrApiKey },
    signal: AbortSignal.timeout(30000),
  });
...
async function handleAdminUsersUpdate(req) {
  ...
  const { userId, role, maxRating, parentId, maxAgeRating } = await req.json();
  ...
  const db = await getDb();
  const updateFields = { role, maxRating: maxRating || '', updatedAt: new Date() };
  if (parentId !== undefined) updateFields.parentId = parentId || null;
  if (maxAgeRating !== undefined) updateFields.maxAgeRating = maxAgeRating || '';
...
// ════════════════════════════════════════════════════════════════════════════
// ██  MISSION 1 : ADMIN TELEMETRY — V0.008
// ════════════════════════════════════════════════════════════════════════════

/**
 * GET /api/admin/telemetry/{userId}
 * Agrège les données de télémétrie d'un utilisateur : temps total, genres cliqués, historique récent.
 * @param {Request} req - Session requise, rôle admin requis. Param: userId dans le chemin
 * @returns {Object} {totalWatchTime, topGenres, recentActivity, searchHistory, favorites}
 */
async function handleAdminTelemetry(req, targetUserId) {
  const session = await getSession(req);
  if (!session) return jsonResponse({ error: 'Non authentifie' }, 401);
  ...
  if (!targetUserId) return jsonResponse({ error: 'userId requis' }, 400);

  const db = await getDb();

  // Total watch time
  const watchAgg = await db.collection('telemetry').aggregate([
    { $match: { userId: targetUserId, action: 'watch' } },
    { $group: { _id: null, totalSeconds: { $sum: '$value' } } },
  ]).toArray();
  const totalWatchTime = watchAgg[0]?.totalSeconds || 0;

  // Top genres by click count
  const genreAgg = await db.collection('telemetry').aggregate([
    { $match: { userId: targetUserId, action: 'click' } },
    { $unwind: '$genres' },
    { $group: { _id: '$genres', count: { $sum: 1 } } },
    { $sort: { count: -1 } },
    { $limit: 10 },
  ]).toArray();
  const topGenres = genreAgg.map(g => ({ genre: g._id, count: g.count }));
  ...
  // User ratings
  const userRatings = await db.collection('telemetry')
    .find({ userId: targetUserId, action: 'rate' })
    .sort({ timestamp: -1 })
    .limit(20)
    .project({ _id: 0 })
    .toArray();
  ...
  // Favorites count
  const favCount = await db.collection('favorites').countDocuments({ userId: targetUserId });

  // User preferences
  const prefs = await db.collection('preferences').findOne({ userId: targetUserId });
  ...
  return jsonResponse({
    ...
    preferences: {
      ...
      rejectedContentIds: (prefs?.rejectedContentIds || []).length,
    },
  });
}
...
/**
 * GET /api/admin/stats
 * Statistiques globales du serveur pour le dashboard admin.
 */
async function handleAdminStats(req) {
  ...
  // Watch time total across all users
  const globalWatchAgg = await db.collection('telemetry').aggregate([
    { $match: { action: 'watch' } },
    { $group: { _id: null, totalSeconds: { $sum: '$value' } } },
  ]).toArray();

  // Most active users
  const activeUsersAgg = await db.collection('telemetry').aggregate([
  ...
  // Top genres globally
  const globalGenreAgg = await db.collection('telemetry').aggregate([
    { $match: { action: 'click' } },
    { $unwind: '$genres' },
    { $group: { _id: '$genres', count: { $sum: 1 } } },
    { $sort: { count: -1 } },
  ...
  // Recent ratings
  const recentRatings = await db.collection('telemetry')
    .find({ action: 'rate' })
    .sort({ timestamp: -1 })
    .limit(10)
    .project({ _id: 0 })
  ...
  return jsonResponse({
    ...
    totalEvents,
    totalFavorites,
    totalWatchTimeHours: Math.round((globalWatchAgg[0]?.totalSeconds || 0) / 3600 * 10) / 10,
    activeUsers: activeUsersAgg.map(u => ({ userId: u._id, eventCount: u.eventCount })),
    topGenres: globalGenreAgg.map(g => ({ genre: g._id, count: g.count })),
    recentRatings,
  });
}

// ════════════════════════════════════════════════════════════════════════════
// ██  MISSION 4 : FAVORIS — V0.008
// ════════════════════════════════════════════════════════════════════════════

/**
 * POST /api/media/favorite
 * Ajoute ou retire un média des favoris.
 * @param {Request} req - Body JSON {contentId, name, type, posterUrl, year, action: 'add'|'remove'}
 */
async function handleMediaFavorite(req) {
  const session = await getSession(req);
  if (!session) return jsonResponse({ error: 'Non authentifie' }, 401);

  const { contentId, name, type, posterUrl, year, genres, action } = await req.json();
  if (!contentId) return jsonResponse({ error: 'contentId requis' }, 400);
  ...
  await db.collection('favorites').updateOne(
    { userId: session.userId, contentId: String(contentId) },
    {
      $set: {
        userId: session.userId,
        contentId: String(contentId),
 ...
 * Retourne la liste des favoris de l'utilisateur courant.
 */
async function handleMediaFavorites(req) {
  const session = await getSession(req);
  if (!session) return jsonResponse({ error: 'Non authentifie' }, 401);
...
/**
 * GET /api/media/is-favorite?id={contentId}
 * Vérifie si un média est dans les favoris de l'utilisateur.
 */
async function handleMediaIsFavorite(req) {
  const session = await getSession(req);
  if (!session) return jsonResponse({ error: 'Non authentifie' }, 401);

  const contentId = new URL(req.url).searchParams.get('id');
  if (!contentId) return jsonResponse({ isFavorite: false });
  ...
  return jsonResponse({ isFavorite: !!fav });
}

// ════════════════════════════════════════════════════════════════════════════
// ██  MISSION 5 : TELEMETRY FEEDBACK (HIDE/NOT INTERESTED)
// ════════════════════════════════════════════════════════════════════════════

/**
 * POST /api/telemetry/feedback
 * Enregistre un feedback négatif (pas intéressé/masquer).
 * Ajoute une pénalité définitive au DagzRank pour cet item.
 * @param {Request} req - Body JSON {contentId, genres?: string[]}
 */
async function handleTelemetryFeedback(req) {
  const session = await getSession(req);
  if (!session) return jsonResponse({ error: 'Non authentifie' }, 401);

  const { contentId, genres = [] } = await req.json();
  if (!contentId) return jsonResponse({ error: 'contentId requis' }, 400);

  const normalizedId = normalizeContentId(contentId);
  if (!normalizedId) return jsonResponse({ error: 'ID invalide' }, 400);

  const db = await getDb();

  // Add to rejectedContentIds in preferences
  await db.collection('preferences').updateOne(
    { userId: session.userId },
    {
      $setOnInsert: { userId: session.userId, onboardingComplete: false },
      $addToSet: { rejectedContentIds: normalizedId },
      $set: { updatedAt: new Date() },
    },
    { upsert: true }
  );

  // Log in telemetry for analytics
  await db.collection('telemetry').insertOne({
    userId: session.userId,
    itemId: normalizedId,
    action: 'hide',
    value: -1,
    genres: (genres || []).filter(Boolean).slice(0, 10),
    timestamp: new Date(),
  });
...
async function handleProfileGet(req) {
  ...
  const db = await getDb();
  const userProfile = await db.collection('users').findOne({ userId: session.userId });
  const prefs = await db.collection('preferences').findOne({ userId: session.userId });
  const favCount = await db.collection('favorites').countDocuments({ userId: session.userId });

  // Watch stats
  const watchAgg = await db.collection('telemetry').aggregate([
    { $match: { userId: session.userId, action: 'watch' } },
    { $group: { _id: null, totalSeconds: { $sum: '$value' } } },
  ]).toArray();

  const ratingCount = await db.collection('telemetry').countDocuments({
    userId: session.userId,
    action: 'rate',
  });
  ...
  return jsonResponse({
    profile: {
      userId: userProfile?.userId || session.userId,
    ...
    stats: {
      favoritesCount: favCount,
      totalWatchTimeHours: Math.round((watchAgg[0]?.totalSeconds || 0) / 3600 * 10) / 10,
 ...
 * Met à jour le profil utilisateur (avatar, préférences).
 */
async function handleProfileUpdate(req) {
  const session = await getSession(req);
  if (!session) return jsonResponse({ error: 'Non authentifie' }, 401);

  const { avatarUrl, favoriteGenres, dislikedGenres } = await req.json();
  const db = await getDb();
  ...
  if (favoriteGenres || dislikedGenres) {
    const updateObj = { updatedAt: new Date() };
    if (favoriteGenres) updateObj.favoriteGenres = favoriteGenres;
    if (dislikedGenres) updateObj.dislikedGenres = dislikedGenres;
...
async function routeGet(parts, req) {
  ...
  if (route.startsWith('admin/telemetry/')) return handleAdminTelemetry(req, parts[2]);
  if (route === 'admin/stats') return handleAdminStats(req);

  if (route === 'media/favorites') return handleMediaFavorites(req);
  if (route === 'media/is-favorite') return handleMediaIsFavorite(req);

  if (route === 'profile') return handleProfileGet(req);
...
async function routePost(parts, req) {
  ...
  if (route === 'telemetry/feedback') return handleTelemetryFeedback(req);
  ...
  if (route === 'media/favorite') return handleMediaFavorite(req);
  if (route === 'profile/update') return handleProfileUpdate(req);
