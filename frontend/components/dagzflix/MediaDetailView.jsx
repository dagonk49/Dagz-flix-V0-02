...
import {
  ...
  Subtitles, AudioLines, Play, PlayCircle, Check, User, Building2,
  Heart, EyeOff,
} from 'lucide-react';
  ...
  const submitRating = useCallback(async (value) => {
    ...
    try {
      await api('media/rate', {
        method: 'POST',
        body: JSON.stringify({ itemId: String(contentId), value, genres: item.genres || [] }),
      });
    ...
    <div className="mb-5 inline-flex items-center gap-3 glass rounded-2xl px-5 py-3">
      ...
      <div className="flex items-center gap-1" onMouseLeave={() => setHoverIdx(-1)}>
        {[1, 2, 3, 4, 5].map((val, idx) => (
          <motion.button key={val} onMouseEnter={() => setHoverIdx(idx)} onClick={() => submitRating(val)} whileHover={{ scale: 1.2 }} whileTap={{ scale: 0.85 }} className="p-0.5 transition-colors">
            <Star className={`w-5 h-5 transition-colors ${idx <= activeIdx ? 'text-amber-400 fill-amber-400' : 'text-white/15'}`} />
          </motion.button>
        ))}
      </div>
      {userRating && <span className="text-amber-300 text-sm font-bold">{userRating}/5</span>}
      {globalAvg && <span className="text-white/30 text-xs ml-1">(moy. {globalAvg}{totalRatings > 0 ? ` · ${totalRatings} vote${totalRatings > 1 ? 's' : ''}` : ''})</span>}
    </div>
  );
}
...
function FavoriteButton({ item }) {
  ...
  useEffect(() => {
    if (!contentId) return;
    cachedApi(`media/is-favorite?id=${contentId}`).then(d => setIsFav(d.isFavorite)).catch(() => {});
  }, [contentId]);
  ...
  const toggle = async () => {
    ...
    try {
      ...
      invalidateCache('media/is-favorite');
    } catch { setIsFav(!newState); }
  ...
  return (
    <motion.button
      ...
      onClick={toggle}
      whileTap={{ scale: 0.85 }}
      className={`p-3 rounded-xl transition-all ${isFav ? 'bg-red-500/20 text-red-400 border border-red-500/30' : 'bg-white/5 text-white/40 border border-white/10 hover:bg-white/10'}`}
    >
...
function HideButton({ item }) {
  ...
  const hide = async () => {
    if (!contentId || hidden) return;
    setHidden(true);
    try {
      await api('telemetry/feedback', {
        method: 'POST',
        body: JSON.stringify({ contentId: String(contentId), genres: item.genres || [] }),
      });
  ...
  return (
    <button
      ...
      onClick={hide}
      disabled={hidden}
      className={`p-3 rounded-xl transition-all ${hidden ? 'bg-gray-500/20 text-gray-400 border border-gray-500/30' : 'bg-white/5 text-white/40 border border-white/10 hover:bg-white/10'}`}
      title="Pas intéressé"
    >
...
function PersonCard({ person }) {
  ...
  const photoUrl = person.Id ? `/api/proxy/image?itemId=${person.Id}&type=Primary&maxWidth=200` : person.photoUrl || null;
  const content = (
    <>
      <div className="w-[120px] h-[120px] rounded-2xl overflow-hidden bg-white/5 ring-1 ring-white/5 mb-2 group-hover:ring-red-500/40 transition-all">
        {photoUrl && !imgErr ? (
          ...
          <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-gray-800 to-gray-900"><User className="w-8 h-8 text-gray-600" /></div>
        )}
      </div>
  ...
  return linkId ? (
    <Link href={`/person/${linkId}`} className={cls}>{content}</Link>
  ) : (
...
export function MediaDetailView({ item, onBack, onPlay, onItemClick }) {
  ...
  const fetchAll = async () => {
    ...
    if (detailId) {
      ...
      } catch { setDetail(item); fi = item; }
    } else { setDetail(item); fi = item; }

    const isSeries = fi?.type === 'Series';
    const fId = fi?.id;
    const tmdbId = fi?.tmdbId || fi?.providerIds?.Tmdb;
    const isTmdbOnly = `${fId || ''}`.startsWith('tmdb-') || (!!fi?.tmdbId && !fi?.providerIds?.Tmdb);
    const canStreamFromJellyfin = !!fId && !isTmdbOnly;
    ...
    if (!isSeries) {
      try {
        ...
        if (r.collection) { setCollection(r.collection); setCollectionItems(r.items || []); }
      } catch { /* ignore */ }
    }
      ...
      <div className="relative -mt-56 z-10 px-6 md:px-16 max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row gap-10">
          ...
          <div className="flex-1 pt-4 min-w-0">
            ...
            <StarRating item={d} />
            {/* Actions: Play, Trailer, Favorite, Hide */}
            ...
            {(d?.people || []).length > 0 && d.people.some(p => p.type === 'Actor') && (
              <div className="mb-8">
                <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-4 flex items-center gap-2"><User className="w-4 h-4" />Casting</h3>
                <div className="flex gap-4 overflow-x-auto hide-scrollbar pb-2">
                  {d.people.filter(p => p.type === 'Actor').slice(0, 20).map((p, i) => (
                    <motion.div key={p.Id || i} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04, duration: 0.3 }}>
                      <PersonCard person={p} />
                    </motion.div>
                  ))}
                </div>
              ...
              <div className="mb-6 flex flex-wrap gap-4">
                {d.people.filter(p => p.type === 'Director').slice(0, 3).map((p, i) => (
                  <span key={`dir-${i}`} className="px-3 py-1.5 rounded-xl bg-blue-500/10 text-blue-300 text-sm border border-blue-500/10">
                    <Clapperboard className="w-3 h-3 inline mr-1" /> {p.name}
                  </span>
                ))}
                {d.people.filter(p => p.type === 'Writer').slice(0, 3).map((p, i) => (
                  <span key={`wrt-${i}`} className="px-3 py-1.5 rounded-xl bg-green-500/10 text-green-300 text-sm border border-green-500/10">
                    <User className="w-3 h-3 inline mr-1" /> {p.name}
                  </span>
                ))}
              </div>
            )}
