...
import { Star, Clapperboard, ChevronLeft, ChevronRight, Loader2, Sparkles, Building2, Heart, EyeOff } from 'lucide-react';
 ...
 */
function quickFavorite(item) {
  const cid = item.id || item.tmdbId;
  if (!cid) return;
  fetch('/api/media/favorite', {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ contentId: String(cid), name: item.name, type: item.type, posterUrl: item.posterUrl, year: item.year, genres: item.genres, action: 'add' }),
  }).catch(() => {});
}
function quickHide(item) {
  const cid = item.id || item.tmdbId;
  if (!cid) return;
  fetch('/api/telemetry/feedback', {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ contentId: String(cid), genres: item.genres || [] }),
  }).catch(() => {});
}
