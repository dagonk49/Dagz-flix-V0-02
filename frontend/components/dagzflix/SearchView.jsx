    ...
    (async () => {
      ...
      try {
        ...
        if (genreFilter || studioFilter) {
          // Genre/studio filter (with or without text) → Jellyfin library (native filtering)
          // Jellyfin matches genres by exact text (language-safe, no conversion needed)
          const params = new URLSearchParams();
          if (typeFilter === 'movie') params.set('type', 'Movie');
          else if (typeFilter === 'tv') params.set('type', 'Series');
          else params.set('type', 'Movie,Series');
          if (genreFilter) params.set('genres', genreFilter);
          if (studioFilter) params.set('studios', studioFilter);
          if (q) params.set('searchTerm', q);
          params.set('limit', '60');
          params.set('sortBy', 'CommunityRating');
          params.set('sortOrder', 'Descending');
          data = await cachedApi(`media/library?${params.toString()}`);
          if (!cancelled) setResults((data.items || []).map(i => ({ ...i, mediaStatus: 5 })));
        } else if (q) {
